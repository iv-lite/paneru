...
| `mouse_drag_display_modifier` | String | *None* | If enabled, holding these keys while left-clicking a tiled window arms a display drag: paneru then moves the window itself from drag motion (grab anywhere — a native edge-resize can no longer win), and crossing a display boundary moves it to that display's strip live (focus follows). The drag arms only when the shortcut is held while left-clicking the window, and the shortcut must stay held throughout; without it, tiled windows pin to their slot and cannot be mouse-moved. While armed, the dragged window is immune to resizing (paneru's resize trigger and native size echoes are both suppressed for it). For example `alt` moves windows between monitors with Alt-drag. Unset by default: ordinary drags snap back to their own strip. With `insert_windows_mid_strip`, the drop lands in the nearest column instead of appending. Throughout an armed drag a filled ghost marks the landing slot on the hovered display. Releasing the button glides a displaced window back into its column (the strip never chases the drop). Moving a window to another display preserves its width ratio up to the target viewport width (maximum ratio 1.0). Diagnose the whole gesture with `RUST_LOG=debug`: arming, pin-vs-transfer, preview show/hide reasons and warp decisions are all logged, and `printstate` gains a `Drag:` section with holders, modifiers, ghost rect and resolved mouse config. |
