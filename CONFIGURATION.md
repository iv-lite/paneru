...
| `mouse_drag_display_modifier` | String | *None* | If enabled, dragging a tiled window across a display boundary while holding these keys moves it to that display's strip live (focus follows). The drag arms only when the shortcut is held while left-clicking the window; without it, tiled windows pin to their slot and cannot be mouse-moved. For example `alt` moves windows between monitors with Alt-drag. Unset by default: ordinary drags snap back to their own strip. With `insert_windows_mid_strip`, the drop lands in the nearest column instead of appending. Throughout an armed drag a filled ghost marks the landing slot on the hovered display. |
