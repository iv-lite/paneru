...
| `left_drag_scrolls_strip` | Boolean | `true` | When enabled, dragging a tiled window with the left mouse button — without the display-drag modifier held — scrolls the workspace strip through the same pipeline as modifier+scroll (same swipe sensitivity, gesture direction, constraints and release behavior; the pointer delta is mapped so the strip tracks the cursor), but only for grabs on the window header (titlebar, or taller AX toolbar where exposed; resize margins excluded) — content grabs keep fully native behavior — and the native drag is swallowed so macOS can't move the window either. Floating windows always keep native behavior. Modifier-held (armed) drags still move and transfer as before. Clicks (press without travel) are unaffected. Only horizontal pointer motion drives anything: vertical travel is dropped so a shaky drag can't pull columns off their slots. Disable if you need native drag behaviors like text selection inside tiled windows. |
| `drag_friction_enabled` | Boolean | `true` | Eases sustained left-drag motion out instead of tracking the pointer 1:1 forever: the further the strip (or armed column) has travelled from the grab anchor, the less each new pixel counts, and a strip held still with the button down is released to the inertia/snap pipeline on its own. Disable for exact 1:1 pointer tracking. |
| `drag_friction_distance_tau_px` | Float (px) | `1200.0` | Distance constant for the exponential falloff: each new dragged pixel counts `exp(-travelled_from_grab / tau)`, clamped to 200–10000. Smaller values damp sooner. |
...
| `experimental_vsync` | Boolean | `false` | Paces the pump to the display's retrace via a per-screen display link instead of fixed 8/16ms sleeps. Experimental: needs macOS 14+, falls back to the sleep ladder when unbound. |
