...
| `mouse_drag_display_modifier` | String | *None* | If enabled, holding these keys while left-clicking a tiled window arms a display drag: paneru then moves the window's whole column itself from drag motion (grab anywhere — a native edge-resize can no longer win, stacked/tabbed mates follow instead of tearing off), and crossing a display boundary moves the column to that display's strip live with grouping intact (focus stays on the grabbed window). The drag arms only when the shortcut is held while left-clicking the window, and the shortcut must stay held throughout. Without the shortcut, title-bar drags scroll the workspace strip instead of moving anything (see `left_drag_scrolls_strip`) — only armed drags reorder (nearest slot on same-display drops) or transfer. While armed, the dragged column is immune to resizing (paneru's resize trigger and native size echoes are both suppressed for it). For example `alt` moves windows between monitors with Alt-drag. Unset by default: ordinary drags scroll the strip. With `insert_windows_mid_strip`, the drop lands in the nearest column instead of appending. A window joining another display is resized to at most the target width (maximum ratio 1.0) at attach time. Throughout an armed drag a filled ghost spanning the dragged column marks the landing slot on the hovered display. Releasing the button glides a displaced window back into its column (the strip never chases the drop). Diagnose the whole gesture with `RUST_LOG=debug`: arming, pin-vs-transfer, preview show/hide reasons and warp decisions are all logged, and `printstate` gains a `Drag:` section with holders, modifiers, ghost rect and resolved mouse config. |
| `left_drag_scrolls_strip` | Boolean | `true` | When enabled, dragging a tiled window with the left mouse button — without the display-drag modifier held — scrolls the workspace strip through the same pipeline as modifier+scroll (same swipe sensitivity, gesture direction, constraints and release behavior; the pointer delta is mapped so the strip tracks the cursor), but only for grabs on the window titlebar (top 28px; resize margins excluded) — content, toolbar and tab grabs keep fully native behavior — and the native drag is swallowed so macOS can't move the window either. Floating windows always keep native behavior. Modifier-held (armed) drags still move and transfer as before. Clicks (press without travel) are unaffected. Only horizontal pointer motion drives anything: vertical travel is dropped so a shaky drag can't pull columns off their slots. Presses anywhere else on a tiled window (content, toolbars, tabs) drive nothing: the holder carries no arming markers, so paint, samplers, overlay passes, snapshot fast-polling and active pacing all stay off — text selection and friends cost nothing beyond press/release bookkeeping and run fully natively. Disable if you need native drag behaviors like text selection inside tiled windows. |
| `drag_friction_enabled` | Boolean | `true` | Eases sustained left-drag motion out instead of tracking the pointer 1:1 forever: the faster the recent motion, the less each new pixel counts, and a strip held still with the button down is released to the inertia/snap pipeline on its own. Slow motion always tracks 1:1 and a pause washes the debt away — friction shapes stripes and windows, never the pointer. Disable for exact 1:1 pointer tracking. |
| `drag_friction_distance_tau_px` | Float (px) | `1200.0` | Scale for the exponential falloff over recent travel: each new dragged pixel counts `exp(-recent_travel / tau)`, clamped to 200–10000. Smaller values damp sooner. Recent travel decays with a 300ms time constant, so only sustained fast motion feels it. |
| `drag_friction_idle_settle_ms` | Integer (ms) | `400` | Stillness after the last drag motion at which a held strip is handed to the inertia/snap pipeline while the button stays down, clamped to 100–5000. Resuming the drag re-drives from the settled offset with no jump. |
| `drag_friction_min_rate` | Float (0.0–1.0) | `0.05` | Floor on the friction factor so a moving pointer never fully glues the strip: even far from the grab anchor each pixel counts at least this much. |
