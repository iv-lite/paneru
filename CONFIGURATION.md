...
| `mouse_drag_display_modifier` | String | *None* | If enabled, dragging a tiled window across a display boundary while holding these keys moves it to that display's strip live (focus follows). For example `alt` moves windows between monitors with Alt-drag. Unset by default: ordinary drags snap back to their own strip. |
