...
/// A native slip mid-scroll (a drag through the press-frame race starts a
/// real OS drag session the app honors over our pushes) must not leave the
/// window disjoint: release re-reads OS truth and snaps it back into its
/// slot while the strip keeps its scroll offset.
#[test]
fn test_scroll_release_resyncs_native_slip() {
    // Window 0 tiles into the (0, 20) slot; drag right without the shortcut.
    let grab = CGPoint::new(200.0, 500.0);
    let commands = vec![
        Event::MenuOpened { window_id: 0 },
        Event::MouseDown {
    ...
    TestHarness::new()
        .with_windows(2)
        .on_iteration(2, move |world, _state| {
            // The strip followed the (100, 0) drag with the column in-slot.
            let entity = find_window_entity(0, world);
            let position = world.get::<Position>(entity).expect("need position").0;
            assert_eq!(position, Origin::new(100, TEST_MENUBAR_HEIGHT));
        })
        .on_iteration(3, move |world, state| {
            // Everything settled: inject the native slip behind paneru's
            // back, mimicking the press-frame race.
            let entity = find_window_entity(0, world);
            let position = world.get::<Position>(entity).expect("need position").0;
            assert_eq!(position, Origin::new(100, TEST_MENUBAR_HEIGHT));
        ...
        .on_iteration(5, move |world, _state| {
            // Release snapped the OS window back into its slot...
            let entity = find_window_entity(0, world);
            let position = world.get::<Position>(entity).expect("need position").0;
            assert_eq!(position, Origin::new(100, TEST_MENUBAR_HEIGHT));
            let window = world.get::<Window>(entity).expect("need window");
